...
func TestInstructionNew(t *testing.T) {
	...
	assert.Equal("[4-5]: s_load_dwordx4 s[8:11], s[2:3], 0x60", inst.Assembly)
...
func TestQueryCreateInstTable(t *testing.T) {
	assert := assert.New(t)

	query := QueryCreateInstTable("test")

	assert.NotEmpty(query, "Empty query")
}
